...
from app.infrastructure.repositories.postgre_person_repository import PostgrePersonRepository

router = APIRouter()


@router.post("", status_code=status.HTTP_201_CREATED, summary="Create Person")
async def create_person(create_person_input_dto: CreatePersonInputDto):
    try:
        logger.info("Received request POST - controller person")
        repository = PostgrePersonRepository()
        uc_create_person = CreatePerson(repository)
        return await uc_create_person.execute(create_person_input_dto)
    except PersonStateUnauthorizedException as psue:
        return JSONResponse(status_code=status.HTTP_409_CONFLICT, content={"error": psue.message})
...
async def get_person(id: int) -> JSONResponse:
    try:
        ...
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"error": pnfe.message})
